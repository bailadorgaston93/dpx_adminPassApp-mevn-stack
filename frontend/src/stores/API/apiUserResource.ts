...
export const connectApiUserResource = async (IS_AUTH: any, BODY: any, EXTRA: any, CLEAN: any, RESPONSE: any) => {
    ...
    await axios({
        ...
        if (BODY.method === "POST" || BODY.method === "PUT") {
            if (BODY.param === "user/create-resource" || BODY.param.includes('user/edit-resource/')) {
                EXTRA.onCloseModal();
                EXTRA.$toast.success(res.data.msg, {
                    position: 'top-right'
                });
